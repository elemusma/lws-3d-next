...
export const metadata: Metadata = {
  ...
  description: 'Our fulfillment policy outlines how we deliver our services, including timelines, responsibilities, and refund terms, including a 14-day money-back guarantee.'
};
        ...
        <div className="max-w-4xl mx-auto px-4 py-16 sm:px-6 lg:px-8">
            ...
            <section key={`section-${sectionIndex}`} className="mb-12">
              ...
              <ol className="list-decimal pl-5 space-y-5">
                {section.items.map((item, itemIndex) => (
                  <li key={`item-${sectionIndex}-${itemIndex}`} className="font-medium">
                    <span>{item.title}</span>
                    <ul className="list-disc pl-5 mt-2 space-y-2">
                      {item.descriptions.map((desc, descIndex) => (
                        <li key={`desc-${sectionIndex}-${itemIndex}-${descIndex}`} className="text-muted-foreground font-normal">
                          {desc}
                        </li>
                      ))}
                    </ul>
          ...
          <section className="mt-16">
            ...
            <p className="text-muted-foreground mb-6">
              If you have any questions or concerns about our Fulfillment Policy, please contact us at:
            </p>
            <div className="flex flex-col sm:flex-row gap-5">
              <a href={`mailto:${policyData.contactInfo.email}`} className="flex items-center gap-2 text-primary hover:text-primary/90 transition-colors duration-200 group">
                <Mail className="h-5 w-5 group-hover:scale-110 transition-transform duration-200" />
                <span className="font-medium">{policyData.contactInfo.email}</span>
              </a>
              <a href={`tel:${policyData.contactInfo.phone.replace(/[^0-9]/g, '')}`} className="flex items-center gap-2 text-primary hover:text-primary/90 transition-colors duration-200 group">
                <Phone className="h-5 w-5 group-hover:scale-110 transition-transform duration-200" />
                <span className="font-medium">{policyData.contactInfo.phone}</span>
              </a>
            </div>
          </section>
        </div>
